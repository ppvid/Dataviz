...
BASE_DIR = Path(__file__).resolve().parent   # 현재 main_hello.py가 있는 폴더
df = pd.read_csv(BASE_DIR / "키워드카운트.csv")
...
BASE_DIR = Path(__file__).resolve().parent  # main_hello.py가 있는 폴더 기준

st.image(BASE_DIR / "워드클라우드.png", use_container_width=True)
st.write(
    "글로벌, 3개 부분, 마마 어워즈 및 여러 걸그룹이 언급되는 걸 보아 최근에 글로벌 시상식인 "
    "마마 어워즈에 나가 수상을 한 것이 이슈가 되었다. 또한 강감독과 매기강 등의 언급을 보면 "
    "케데헌의 감독의 역할이 주목받고 있음을 알 수 있다."
)

st.write("## 3. 뉴스 본문 키워드 네트워크 시각화")
st.caption("출처: 네이버 뉴스")

st.write("### 네트워크 시각화")
st.image(BASE_DIR / "네트워크그래프.png", use_container_width=True)

st.write("### circular 네트워크 시각화")
st.image(BASE_DIR / "circulara_네트워크.png", use_container_width=True)
